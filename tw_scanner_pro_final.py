...
# -*- coding: utf-8 -*-
"""
TW Scanner Pro (Final Ultimate v3.2 - Long Term MA Support)
Update: Added MA120 (Half-Year) and MA240 (Yearly) lines to charts
"""
...
def download_history(symbol, days=360, interval="1d"):
    ...
    # [Modified] 增加歷史長度以確保 MA240 計算無誤
    start = end - timedelta(days=int(max(days, 365) * 1.8))
    ...
    try:
        # [Modified] 預設抓 2 年確保年線
        df = yf.Ticker(symbol).history(period="2y", interval=interval, auto_adjust=False)
        if isinstance(df.columns, pd.MultiIndex): df.columns = df.columns.get_level_values(0)
...
def plot_universal_chart(df, code, name, category, group, outdir, strategy, add_plots=None):
    try:
        os.makedirs(outdir, exist_ok=True)
        # 繪圖範圍
        plot_df = df.iloc[-180:].copy() if len(df)>180 else df.copy()
        
        # [Modified] 計算所有均線 (包含 120, 240)
        # 注意：要在原 df 計算完後再切片，以確保數值正確，但這裡簡化直接檢查
        # 若 df 夠長，我們直接在 plot_df 上畫 (前提是 plot_df 包含足夠歷史? 不行)
        # 正確做法：在傳入的 df 上先算好，再切片
        ...
        # [Modified] 加入 MA120 (Blue) 和 MA240 (Red)
        ap = [
        ...
        if add_plots: 
            ...
            try: 
                marker_y = plot_df["High"].iloc[-1]*1.01
                if "pullback" in strategy.lower(): marker_y = plot_df["Low"].iloc[-1]*0.99
                
                ap.append(mpf.make_addplot(pd.Series([np.nan]*(len(plot_df)-1)+[marker_y], index=plot_df.index), scatter=True, markersize=50, marker="v" if "pullback" not in strategy.lower() else "^", color='red'))
            except: pass
        ...
        mpf.plot(plot_df, type="candle", volume=True, addplot=ap, title=title, style="yahoo", 
...
def plot_wave3_chart(df, res, code, name, category, group, outdir):
    try:
        ...
        plot_df = df.iloc[-260:].copy() if len(df)>260 else df.copy()
        
        date_str = [str(t.date()) for t in plot_df.index]
        def mk(d, p): 
            s = pd.Series(np.nan, index=plot_df.index)
            if d in date_str: s.iloc[date_str.index(d)] = p
            return s
        
        # [Modified] 加入 MA120/240
        ap = [
            mpf.make_addplot(plot_df["MA5"], width=1, color='fuchsia'), 
            mpf.make_addplot(plot_df["MA20"], width=1, color='orange'), 
            mpf.make_addplot(plot_df["MA60"], width=1, color='green'),
            ...
            mpf.make_addplot(pd.Series(res["neckline"], index=plot_df.index), linestyle="--", color='gray')
        ]
...
def make_indicators(df, ma_short, ma_long, vol_window, bb_window):
    for w in (5,10,20,60,120,240): df[f"MA{w}"] = df["Close"].rolling(w).mean()
    delta = df["Close"].diff()
...
def scan_ma_cross(df, golden): return False
...
def scan_monitor(df): return True 
...
def scan_pullback_ma20(df, threshold_pct, window_days):
    if len(df) < max(60, window_days + 2): return False
    ...
    if ma20 <= ma60: return False

    upper_bound = ma20 * (1 + threshold_pct)
    is_touching = low <= upper_bound
    is_holding = close >= ma20 
    
    if not (is_touching and is_holding): return False

    start_idx = -(window_days + 1)
    end_idx = -1
    subset = df.iloc[start_idx:end_idx].copy()
    
    has_breakout = False
    for i in range(len(subset)):
        idx_in_df = df.index.get_loc(subset.index[i])
        curr_c = df["Close"].iloc[idx_in_df]
        curr_ma = df["MA20"].iloc[idx_in_df]
        prev_c = df["Close"].iloc[idx_in_df - 1]
        prev_ma = df["MA20"].iloc[idx_in_df - 1]
        
        if curr_c > curr_ma and prev_c <= prev_ma:
            has_breakout = True
            break
...
def run_strategy(universe, args, outdir, name):
    ...
    for _, r in universe.iterrows():
        ...
        elif name == "breakout_fade": ok = scan_breakout_fade(df, args.bf_lookback, args.bf_vol_ratio)
        elif name == "pullback_ma20": ok = scan_pullback_ma20(df, args.pullback_threshold, args.pullback_days)
        elif name == "wave3": 
...
def main():
    ...
    # Breakout Fade
    ap.add_argument("--bf-lookback", type=int, default=60)
    ap.add_argument("--bf-vol-ratio", type=float, default=1.5)

    # Pullback
    ap.add_argument("--pullback-threshold", type=float, default=0.015)
    ap.add_argument("--pullback-days", type=int, default=5, help="Breakout window days for pullback")
