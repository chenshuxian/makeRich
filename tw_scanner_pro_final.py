...
# -*- coding: utf-8 -*-
"""
TW Scanner Pro (Final Ultimate v2.6)
Features: Monitor Mode (No-Filter) + Group Filter + Filename Logic + Universal Charts
2025 Integrated Version
"""

import os, sys, argparse, concurrent.futures as futures, re, time, logging
...
def build_intraday_overrides(universe_df, tz_name, session_start_str, minute_interval, days, threads):
    ...
    def _one(row):
        try:
            ...
            if not min_df.empty:
                s = pd.Series({"Open": min_df["Open"].iloc[0], "High": min_df["High"].max(), "Low": min_df["Low"].min(), "Close": min_df["Close"].iloc[-1], "Volume": min_df["Volume"].sum()})
    ...
    with futures.ThreadPoolExecutor(max_workers=threads) as ex: list(ex.map(_one, universe_df.itertuples(index=False)))
...
def plot_universal_chart(df, code, name, category, group, outdir, strategy, add_plots=None):
    try:
        ...
        if add_plots: ap.extend(add_plots)
        elif strategy != "monitor": # Monitor mode doesn't need markers
            try: ap.append(mpf.make_addplot(pd.Series([np.nan]*(len(df)-1)+[df["High"].iloc[-1]*1.01], index=df.index), scatter=True, markersize=50, marker="v", color='red'))
            except: pass
        ...
        mpf.plot(df, type="candle", volume=True, addplot=ap, title=title, style="yahoo", figratio=(16,9), figscale=1.1, savefig=dict(fname=os.path.join(outdir, fname), dpi=120, bbox_inches="tight"))
...
def plot_wave3_chart(df, res, code, name, category, group, outdir):
    try:
        ...
        def mk(d, p): 
            ...
            return s
        ap = [
            mpf.make_addplot(df["MA5"], width=1, color='fuchsia'), mpf.make_addplot(df["MA20"], width=1, color='orange'), mpf.make_addplot(df["MA60"], width=1, color='green'),
...
def scan_ma_cross(df, golden): p, c = df.iloc[-2], df.iloc[-1]; return (p["MA_S"]<=p["MA_L"] and c["MA_S"]>c["MA_L"]) if golden else (p["MA_S"]>=p["MA_L"] and c["MA_S"]<c["MA_L"])
...
def run_strategy(universe, args, outdir, name):
    ...
    for _, r in universe.iterrows():
        ...
        # Dispatch
        if name == "monitor": ok = scan_monitor(df) # Always True
    ...
    pd.DataFrame(rows).to_csv(out_csv, index=False, encoding="utf-8-sig") if rows else pd.DataFrame(columns=["code"]).to_csv(out_csv, index=False)
    return out_csv
